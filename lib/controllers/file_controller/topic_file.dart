...
class TopicFile {
  ...
  void update(Map<String, dynamic> oldValue, Map<String, dynamic> newValue) =>
      _process(
        oldValue,
        _deleteFromTopics,
        op2: _addToTopic,
  ...
  void createDatabase() async {
    ...
    for (TopicModel topic in topics)
      TopicDatabase.topicDatabase.insert(topic.toMap());
  }
  ...
  /// Private method for encoding and writing the contents of [topics] into
  ...
  /// Private method to manipulate the contents of the tile topic.json
  ///
  ...
  void _process(
    ...
    print("Result of operation is: ${op1(TopicModel.fromMap(value))}");
    if (newValue != null) op2(TopicModel.fromMap(newValue));
    _write();
  }

  /// Private method to check and delete appropriate topic from the list [topics].
  ///
  ...
  /// Private method to check and add topic to the list [topics].
  ///
  /// The method compares the id to identify appropriate topic. If a topic with
  /// the same id exists in the list then [topic] isn't added.
